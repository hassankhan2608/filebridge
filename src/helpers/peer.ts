...
let pingListeners: ((peerId: string) => void)[] = [];
...
export const PeerConnection = {
    ...
    onPingReceived: (callback: (peerId: string) => void) => {
        pingListeners.push(callback);
    },

    removePingListener: (callback: (peerId: string) => void) => {
        pingListeners = pingListeners.filter(listener => listener !== callback);
    }
}